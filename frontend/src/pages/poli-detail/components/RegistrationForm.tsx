...
export function RegistrationForm({ selectedDoctor, patient, selectedDate, poliId }: RegistrationFormProps) {
  const isDisabled = !selectedDoctor;
  const navigate = useNavigate();
  const { setRegistrationResult } = useRegistrationFlowStore();
  ...
  // Smooth scroll to form when doctor is selected
  useEffect(() => {
    if (selectedDoctor) {
  ...
  }, [submitMutation.isSuccess, submitMutation.data, setRegistrationResult, navigate]);
        ...
        {isDisabled ? (
          <div className="text-center py-8">
            ...
            <p className="text-sm" style={{ color: "var(--color-text-secondary)" }}>
              Silakan pilih dokter dari jadwal di atas untuk melanjutkan pendaftaran
            </p>
          </div>
        ) : (
          <>
